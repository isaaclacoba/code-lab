...
public class CompilerService
{
    ...
    internal static string? FriendlyHint(string diagnosticId) => diagnosticId switch
    {
        ...
        "CS0164" => "This label is never jumped to, so it does nothing at all. Labels only mean something to a 'goto', and if nothing goes there the label is just noise. Remove it.",
    ...
    {
        "CS1002" or "CS1513" or "CS1514" or "CS1026" or "CS1003" or "CS1525" => "The compiler does not read your code the way you read a sentence. It reads structure: every '{' has to close, every '(' has to close, every statement has to end. When one of them is missing the compiler keeps reading anyway, and everything after it lands in the wrong place - which is why one missing character can produce a whole page of complaints. That is the reason the advice is always the same: fix the FIRST one, then run again. Most of the rest disappear on their own.",
        "CS0103" or "CS0246" => "C# looks names up, it never guesses. For a name to work here it has to be declared somewhere the compiler can see from this line, and spelled identically - 'catCount' and 'CatCount' are two different names. Where it was declared matters as much as the spelling: a variable declared inside an if block or a loop stops existing the moment that block closes. So there are only ever two questions to ask - is it spelled the same, and is it still in scope right here?",
        "CS0117" => "A type is a promise about what it offers, and you can only ask an object for something its type actually declares. If a List has no 'Push' then wanting one will not create it. This is the compiler working for you: it checks every call you write BEFORE the program runs, rather than letting it fail in front of a user. When you are not sure what a type gives you, ask the editor instead of guessing - type the dot and it lists everything that type can do.",
        "CS0759" => "A method is identified by more than its name. The parameters and their types are part of its identity, so 'Feed(Cat)' and 'Feed(Cat, int)' are two different methods as far as C# is concerned. When a signature has to line up with one declared elsewhere, every part has to match: the name, the return type, and the parameter list in order.",
        ...
        "CS0219" or "CS0168" or "CS0169" or "CS0414" or "CS0164" => "Unused things are evidence. A variable or field that is written but never read means the code that was supposed to consume it is missing, or that a refactor left debris behind. Neither is harmless: the next reader has to work out whether it matters, and dead state is where stale, wrong values hide. Delete it or finish it - do not leave it.",
